...
builder.entity("bullet-${Math.random()}") {
	...
	component(new DisablerComponent(new GenericHitComponent("bullethitComponent"))){
		...
		property("predicate",{Predicates.and(
				{ball -> ball.collisionMask == entity.collisionMask} as Predicate, // 
				SlickEntityPredicates.isNear(entity.position, (float)entity.radius * 2-3), //
				{ball -> ball.alive} as Predicate //
				)})
	...
	component(utils.components.genericComponent(id:"bulletHitHandler", messageId:["bulletHit"]){ message ->
		...
		utils.messageQueue.enqueue(ChildrenManagementMessageFactory.removeEntity(entity))
	})
}
